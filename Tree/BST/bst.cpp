...
public:
    ...
    void genFromPre(Node* &root, int pre[], int n);
...
void Tree::genFromPre(Node* &root, int pre[], int n){
    std::stack<Node*> st;
    int i = 0;
    root = new Node(pre[i]);
    i++;
    ...
    while (i < n) {
        if (pre[i] < p->data) {
            ...
            if (!st.empty() && (pre[i] > p->data) && (pre[i] < st.top()->data)) {
...
int main() {
    ...
    int arr[] ={30,21,10,15,25,40,50,45};
    Tree t;
    Node* root = NULL;
